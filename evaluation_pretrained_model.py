...
def multi_scale_inference(model, image, scales=[1, 1.1, 1.2, 1.3, 1.4, 1.5], flip=False):
    ...
    for scale in scales:
        ...
        if scale <= 1.0:
            ...
            rows = int(np.ceil(1.0 * (new_h - crop_size) / stride_h)) + 1
            cols = int(np.ceil(1.0 * (new_w - crop_size) / stride_w)) + 1
    ...
    for seed_idx, seed in enumerate([42, 43, 44, 45, 46]):
        ...
        for file_idx, img_path in enumerate(tqdm(list_img_path)):
            ...
            with torch.no_grad():
                ...
                vis.save(f'./submit/task1/class1/{pname}/fig/{filename}_{args.target}_{seed}.png')
...
def inference_class_2(args, pname):
    ...
    for seed_idx, seed in enumerate([42, 43, 44, 45, 46]):
        ...
        for file_idx, img_path in enumerate(tqdm(list_img_path)):
            ...
            vis = Image.fromarray(img)
            vis.save(f'./submit/task1/class2/{pname}/fig/{filename}_origin.png')
...
def inference_class_3(args, pname):
    ...
    ensemble_preds = np.zeros([5, 65, 3, 1024, 1024])
    for seed_idx, seed in enumerate([42, 43, 44, 45, 46]):
        lm = Task1LM.load_from_checkpoint(glob(f'{args.model_dir}/class_3/{pname}/{seed}_*.ckpt')[0])
        lm.eval().cuda()
        ...
        for file_idx, img_path in enumerate(tqdm(list_img_path)):
            ...
            with torch.no_grad():
                ...
                vis = Image.fromarray(pred[:,:,1]*255).convert('RGB')
                vis.save(f'./submit/task1/class3/{pname}/fig/{filename}_{args.target}_{seed}.png')
    ...
    for z, img_path in enumerate(list_img_path):
        filename = os.path.basename(img_path).split('.')[0]
        vis = Image.fromarray(ensemble_preds[z, 1,:,:]*255).convert('RGB')
        vis.save(f'./submit/task1/class3/{pname}/fig/{filename}_{args.target}_ensemble.png')
    ...
    #         e_pred = np.zeros([1024, 1024, 3])
    #         for z, img in enumerate([img0, img90, img180, img270]):
    #             pred = multi_scale_inference(lm, img)
...
def get_dice_score(b1, b2):
    intersection = np.sum(b1 * b2)
    union = np.sum(b1) + np.sum(b2)
    if union == 0:
        return 1.
...
def post_edit_class2(args, pname):
    ...
    for img_path in tqdm(list_img_path):
        ...
        Image.fromarray(post_mask).save(f"./submit/task1/class2/{pname}/fig/{filename}_{args.target}_post.png")
